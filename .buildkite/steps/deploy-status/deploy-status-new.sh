...
json_template_file="./assets/template-new.json"



# update json file
jq '.deployments |= (map(if .application == ":bison: Bison" and .environment == "Development" then .deployed_version = ":github: BLAH" | .new_version = ":github: BLOO" | .deployment_status = "Success" | .deployment_progress = 10 | .last_updated = "" | .buildkite_job = "Buildkite Job" | .application_link = "Application Link" else . end))' $json_template_file > ./assets/annotation-new.json
